...
resource "esxi_guest" "vmtest01" {
  ...
  disk_store         = "datastoreHote2"       # Required, Specify an existing Disk Store
  network_interfaces = [
    {
      virtual_network = "HA Network"  # Required for each network interface, Specify the Virtual Network name.
    },
  ]
}
