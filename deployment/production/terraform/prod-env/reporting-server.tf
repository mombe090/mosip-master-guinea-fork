resource "vsphere_virtual_machine" "prod_reportsrver" {
  name             = "prod.reportsrver${var.guest_name_suffix}"
...
provisioner "file" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      format("%s %s", "sudo /tmp/extra_vm.sh", "prod-reportsrver")
    ]
  }
  connection {
      type     = "ssh"
